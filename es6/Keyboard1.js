...
var
	...
	handleTrackMessage = function(e)
	{
		...
		{
			let u = tryParseJSON(jsonMessage),
				u1 = Object.keys(u).map(function(_) { return u[_]; }),
				uint8Array = new Uint8Array(u1);

			sendMIDIMessage(uint8Array);
		}

		// sends all the trk's silent Messages at once.
		function resetTrack(silentTrkMessages)
		{
			let msgsObjArray = tryParseJSON(silentTrkMessages),
				nMessages = msgsObjArray.length;

			for(let i = 0; i < nMessages; ++i)
			{
				let u = msgsObjArray[i],
					u1 = Object.keys(u).map(function(_) { return u[_]; }),
					uint8Array = new Uint8Array(u1);
		...
		function workerHasCompleted(trackIndex)
		{
			...
			{
				let trackWorker = trackWorkers[i];
		...
		{
			case "midiMessage":
				sendMessage(e.data.midiMessage);
				break;
			case "trkStopped":
				...
				resetTrack(e.data.silentTrkMessages);
				break;
			case "workerCompleted":
				...
				workerHasCompleted(e.data.trackIndex);
				break;
			default:
				break;
		}
	...
	init = function(inputDeviceArg, outputDeviceArg, tracksData, reportEndOfPerfCallback, reportMsPosCallback)
	{
		...
		moveMidiRestMomentsToPrecedingMidiChords(tracksData.outputTracks);

		initChannelResetMessages(tracksData.outputTracks.length);
	...
	play = function(trackIsOnArray, startRegionIndex, startMarkerMsPosInScore, endRegionIndex, endMarkerMsPosInScore, baseSpeed, recording)
	{
		...
		{
			var i, nTracks = outputTracks.length, track, regionStartMsPositionsInScore;
			...
			regionStartMsPositionsInScore = [startMarkerMsPosInScore]; // replace by the above function when regions have been properly implemented.
			...
			for(i = 0; i < nTracks; ++i)
			{
				if(trackIsOnArray[i])
				{
					...
					//  The following code replaced by the above line (1.9.2018). Compare with Sequence.play().
		...
		function initPlay(trackIsOnArray, keyInstantIndices, instants, trackWorkers, startMarkerMsPosInScore, endMarkerMsPosInScore, baseSpeed)
		{
			...
			function setInstants(instants, inputTracks, trackIsOnArray, startMarkerMsPosInScore, endMarkerMsPosInScore)
			{
				...
				function getNotesMoments(inputTracks, trackIsOnArray, startMarkerMsPosInScore, endMarkerMsPosInScore)
				{
					...
					for(inputTrackIndex = 0; inputTrackIndex < nInputTracks; ++inputTrackIndex)
					{
						...
						{
							previousChordTrkOptions = new TrkOptions();
							...
							{
								if(inputObjects[ioIndex] instanceof InputChord)
								{
			...
			function setSeqsAndTrackWorkers(instants, trackWorkers, outputTracks, baseSpeed)
			{
				...
				function initTrackWorkers(trackWorkers, outputTracks, baseSpeed)
				{
					...
					for(i = 0; i < outputTracks.length; i++)
					{
						if(trackIsOnArray[i] === true)
						{
							...
							{
								console.log('worker.onmessageerror called.');
							};
				...
				function setSeq(msPosition, noteOnOrOff, trackWorkers)
				{
					...
					{
						noteOnOrOff.seq = new Seq(msPosition, noteOnOrOff.seqDef, trackWorkers);
					}
				}
...
export class Keyboard1
{
	constructor()
	{
		...
		this.handleMIDIInputEvent = handleMIDIInputEvent;
	}
