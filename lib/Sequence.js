...
*      // fromMsPositionInScore, toMsPositionInScore: the part of the sequence to play (not including toMsPositionInScore)
...
*               reportEndOfSpanCallback, reportMsPositionInScoreCallback)
*
    ...
    Sequence.prototype = (function (window)
    {
        ...
        nextMoment = function ()
        {
            ...
            if (!stopped && !paused)
            {
                ...
                else
                {
                    ...
                    nextMomt.timestamp = nextMomt.timeReSequence() + sequenceStartTime;
                }
            }
        ...
        tick = function ()
        {
            ...
            while (delay <= PREQUEUE)
            {
                ...
                if (currentMoment.messages.length > 0) // rest moments can be empty (but should be reported above) 
                {
                    ...
                    {
                        // The moments are recorded with their (absolute DOMHRT) timestamp values.
                        ...
                        // The moment.timestamps do not need to be restored to their original values,
                        // because they will be re-assigned next time nextMoment() is called.
                        ...
                        recordingSequence.tracks[currentMoment.messages[0].channel()].addTimestampedMoment(currentMoment);
                    }
                }
        ...
        runFrom = function (moment, now)
        {
            if (isStopped() || isPaused())
            {
                ...
                sequenceStartTime = now - moment.timeReSequence();
