...
MIDI_API.sequence = (function (window)
{
    ...
    Sequence.prototype = (function (window)
    {
        ...
        endMarkerMsPositionInScore = -1,  // set in playSpan(), used by nextEvent()
        startNow = -1,  // set in playSpan(), used by nextEvent()
        lastSequenceMsPositionInScore = -1, // set in playSpan(), used by nextEvent()
        recordingSequence, // set in playSpan() and resume(), used by tick()
        lastReportedMsPosition = -1, // set by tick() used by nextEvent()
        msPositionToReport = -1,   // set in nextEvent() and used by tick()
            ...
            function nextMoment()
            {
                ...
                for (i = 0; i < nTracks; ++i)
                {
                    ...
                    if (track.isPerforming && track.currentIndex <= track.toIndex) // toIndex is the last valid index
                    {
                        ...
                        if (moment.msPositionInScore < minTimestamp)
                        {
                            nextMomt = moment;
                            minTimestamp = moment.msPositionInScore;
                ...
                if (nextMomt !== null)
                {
                    ...
                    if (nextMomt.msPositionInScore === endMarkerMsPositionInScore || nextMomt.msPositionInScore === lastSequenceMsPositionInScore)
                    {
                        // Do not actually perform the last moment in a span or sequence, but call stop() (below).
                        nextMomt = null;
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        // the position will be reported by tick() when the next event is sent.
        ...
        //      lastSequenceMsPositionInScore // the largest msPositionInScore in any track (i.e. the end of the sequence)
        ...
        tick = function ()
        {
            ...
            while (delay <= PREQUEUE)
            {
                ...
                {
                    // evt.timestamps are currently measured from the start of the _score_, but
                    ...
                    evt.timestamp += domhrtMsOffsetAtStartOfSequence;
                    // evt.timestamps are absolute DOMHRT time here.
                    midiOutputDevice.send(evt.data, evt.timestamp);

                    if (recordingSequence !== undefined && recordingSequence !== null)
                    {
                        // The evt.timestamps are recorded with absolute DOMHRT values here.
                        // These values are set relative to the start of the _performance_
                        // (i.e. the value of the earliest timestamp in the recording is
                        // subtracted from all the timestamps in the recording) before
                        // saving them in a Standard MIDI File.
                        // After saving the recording, the evt.timestamps are reset to their
                        // original values (i.e. moment.msPositionInScore).
        ...
        resume = function ()
        {
            ...
            {
                setState("running");
                domhrtMsOffsetAtStartOfSequence = window.performance.now() - currentMoment.events[0].timestamp;
        ...
        // the timestamp used when sending Events. The timestamp can change dynamically
        // during an assisted performance (when using durations which are relative to the
        // live performer's durations).
        playSpan = function (midiOutDevice, fromMsPositionInScore, toMsPositionInScore, trackIsOnArray, recording, reportEndOfSeq, reportMsPosition)
        {
            var
            fromTimestamp;
            ...
            function setTracks(trackIsOnArray, msOffsetFromStartOfScore, toMsPositionInScore)
            {
                ...
                function setTrackAttributes(tracks)
                {
                    var
                ...
                // Sets the timestamps of the contained events
                // so that they are relative to the span.
                function adjustTimestamps(tracks)
                {
                    var i, j, k, moment, track, nTracks = tracks.length, nEvents, originTimestamp;

                    function getFirstTimestamp(tracks)
                    {
                        var
                        i, nTracks = tracks.length,
                        firstTrackTimestamp, firstTimestamp = Number.MAX_VALUE;
                        ...
                        for (i = 0; i < nTracks; ++i)
                        {
                            ...
                            {
                                firstTrackTimestamp = track.moments[track.fromIndex].events[0].timestamp;
                                firstTimestamp = (firstTimestamp < firstTrackTimestamp) ? firstTimestamp : firstTrackTimestamp;
                            }
                        }
                        return firstTimestamp;
                    }

                    originTimestamp = getFirstTimestamp(tracks);
                    if (originTimestamp > 0)
                    {
                        for (i = 0; i < nTracks; ++i)
                        {
                            ...
                            for (j = track.fromIndex; j < track.toIndex; ++j)
                            {
                                ...
                                for (k = 0; k < nEvents; ++k)
                                {
                                    moment.events[k].timestamp -= originTimestamp;
                                }
                            }
                        }
                    }
                ...
                setTrackAttributes(that.tracks);

                adjustTimestamps(that.tracks);
            }

            // Sets lastSequenceMsPositionInScore to the largest moment.msPositionInScore in any track (i.e. the end of the sequence)
            function setLastSequenceMsPositionInScore()
            {
                var i, nTracks = that.tracks.length, track,
                lastTrackMsPositionInScore;

                lastSequenceMsPositionInScore = -1;
                ...
                for (i = 0; i < nTracks; ++i)
                {
                    ...
                    {
                        lastTrackMsPositionInScore = track.moments[track.moments.length - 1].msPositionInScore;
                        lastSequenceMsPositionInScore = (lastSequenceMsPositionInScore > lastTrackMsPositionInScore) ?
                            lastSequenceMsPositionInScore : lastTrackMsPositionInScore;
                    }
                }
            }

            // Returns the earliest event.timestamp in this sequence.
            function getEarliestTimestamp()
            {
                var
                tracks = that.tracks, nTracks = tracks.length,
                i, ts, timestamp = Number.MAX_VALUE;

                for (i = 0; i < nTracks; ++i)
                {
                    ts = tracks[i].moments[0].events[0].timestamp;
            ...
            setTracks(trackIsOnArray, fromMsPositionInScore, toMsPositionInScore);
            endMarkerMsPositionInScore = toMsPositionInScore;
            lastReportedMsPosition = -1;

            setLastSequenceMsPositionInScore();
            setState("running");

            maxDeviation = 0; // for console.log

            startNow = window.performance.now();
            fromTimestamp = getEarliestTimestamp();
            domhrtMsOffsetAtStartOfSequence = startNow - fromTimestamp;
        ...
        {
            var i, nTracks = this.tracks.length, track,
                j, nMoments, moment,
                k, nEvents;
