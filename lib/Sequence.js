...
*
*  Sequence.js
*  The JI_WEB_MIDI_API.sequence namespace which defines the
...
*      addTrack()
* 
*      // Set the state of the Sequence. The argument can be "stopped", "paused" or "running".
*      setState(string)  
*
...
*      // trackIsOnCallback is a callback. trackIsOnCallback(trackIndex) returns a boolean
*      //     which determine whether the track will be played or not.
...
*      playSpan(midiOutDevice, fromMs, toMs, trackIsOnCallback,
...
JI_WEB_MIDI_API.namespace('JI_WEB_MIDI_API.sequence');

JI_WEB_MIDI_API.sequence = (function (window)
{
    "use strict";
    var 
    ...
    // An empty sequence is created. It contains an empty array of JI_WEB_MIDI_API.track.Tracks.
    ...
    Sequence.prototype = (function ()
    {
        ...
        stop = function ()
        {
            ...
            {
                setState("stopped");
                reportEndOfSequence(recordedSequence, endMarkerTimestamp, false); // endMarkerTimestamp is the toMs argument to playSpan()
            }
        },
        ...
        nextEvent = function ()
        {
            ...
            function nextMoment()
            {
                ...
                if (nextMomt !== null)
                {
                    ...
                    {
                        that.tracks[nextMomentTrackIndex].currentIndex++;
        ...
        tick = function ()
        {
            ...
            while (delay <= PREQUEUE)
            {
                ...
                {
                    recordedSequence.tracks[event.channel()].addEvent(event);
        ...
        //
        // trackIsOnCallback(trackIndex) returns a boolean which determines whether the track
        ...
        playSpan = function (midiOutDevice, fromMs, toMs, trackIsOnCallback, reportEndOfSeq, reportMsPosition)
        {
            var i, nTracks = that.tracks.length;
            ...
            function setTracks(trackIsOnCallback, msOffsetFromStartOfScore, toMs)
            {
                ...
                for (i = 0; i < nTracks; ++i)
                {
                    ...
                    {
                        track.isPerforming = trackIsOnCallback(i);
                    }
            ...
            recordedSequence = new Sequence(that.msPositionInScore);
            for (i = 0; i < nTracks; ++i)
            {
            ...
            reportEndOfSequence = reportEndOfSeq; // can be null
            reportMsPositionInScore = reportMsPosition;

            setTracks(trackIsOnCallback, fromMs, toMs);
        ...
        {
            var nTracks, i, j, k, track, trackLength, moment, events, nEvents, timestamp;
        ...
        getSubsequences = function (livePerformersTrackIndex)
        {
            ...
            function fillSubsequences(subsequences, moments)  // 'base' function in outer scope.
            {
                ...
                for (subsequencesIndex = 0; subsequencesIndex < nSubsequences; ++subsequencesIndex)
                {
                    ...
                    track = new JI_WEB_MIDI_API.track.Track();
            ...
            subsequences = getEmptySubsequences(that.tracks[livePerformersTrackIndex]);

            for (trackIndex = 0; trackIndex < nTracks; ++trackIndex)
            {
                fillSubsequences(subsequences, that.tracks[trackIndex].moments); // 'base' function in outer scope
            }
        ...
        appendFinalBarlineMoment = function (track, sequenceMsPositionInScore, timestamp)
        {
            ...
            finalBarlineMoment = new JI_WEB_MIDI_API.moment.Moment(timestamp);
            finalBarlineMoment.restStart = true;
            // that event will never be sent, because it is given an isEmpty attribute
        ...
        {
            var returnSeq = new Sequence(that.msPositionInScore),
                t, nTracks = that.tracks.length, track,
                newTrack, nMoments, momentToAppend, iMom, limit;

            if (that.restSubsequence === undefined)
            {
                throw "Error: that must be a restSequence.";
            }

            returnSeq.restSubsequence = true;
            limit = toMs - that.msPositionInScore;

            for (t = 0; t < nTracks; ++t)
            {
                newTrack = new JI_WEB_MIDI_API.track.Track();
                track = that.tracks[t];
                ...
                appendFinalBarlineMoment(newTrack, that.msPositionInScore, limit);
        ...
        afterSplit = function (fromMs)
        {
            ...
            if (that.msPositionInScore >= fromMs || that.restSubsequence === undefined)
            {
                throw "Error: that must be a restSequence which begins before the split point.";
            }
            ...
            for (t = 0; t < nTracks; ++t)
            {
                ...
                newMoment = new JI_WEB_MIDI_API.moment.Moment(0);
                newMoment.restStart = true;
                // that event will never be sent, because it is given an isEmpty attribute
                newEvt = new Event(CMD.NOTE_OFF + t, 0, 0, 0); // newEvt.timestamp = 0;
                newEvt.msPositionInScore = that.msPositionInScore;
                newEvt.isEmpty = true;
                ...
                if (nMoments > 0)
                {
                    momentI = indexOfLastMomentBeforeFromMs(track.moments, fromMs - that.msPositionInScore);
                    if (momentI === undefined)
                    {
                        // track.moments[0].timestamp was greater than (fromMs - that.msPositionInScore)
                        // i.e. copy *all* the subsequent events to the new track.
                        momentI = 0;
                    }
                    else if (track.moments[momentI].timestamp + that.msPositionInScore < fromMs)
                    {
                        ++momentI;
                    }
                    for (iMom = momentI; iMom < nMoments; ++iMom)
                    {
                        ...
                        newMoment = new JI_WEB_MIDI_API.moment.Moment(moment.timestamp + that.msPositionInScore - fromMs);
                        if (moment.restStart !== undefined)
                        {
                            newMoment.restStart = true;
                        }
            ...
            overrideSoloPitch, overrideOtherTracksPitch, overrideSoloVelocity, overrideOtherTracksVelocity)
        {
            ...
            track = that.tracks[soloTrackIndex], event, lowestNoteOnEvt, pitchDelta, velocityDelta;
            ...
            function adjustTracks(NOTE_ON_CMD, soloTrackIndex, pitchDelta, velocityDelta,
                overrideSoloPitch, overrideOtherTracksPitch, overrideSoloVelocity, overrideOtherTracksVelocity)
            {
                var nTracks = that.tracks.length, i, j, k, nMoments, moment, nEvents;
