...
*
*  jiSequence.js
*  The WEB_MIDI_LIB.sequence namespace which defines the
*    Sequence(msPositionInScore) empty sequence constructor. 
*/

WEB_MIDI_LIB.namespace('WEB_MIDI_LIB.sequence');

WEB_MIDI_LIB.sequence = (function (window)
{
    "use strict";
    var
    MIDIEvent = WEB_MIDI_LIB.midiEvent.MIDIEvent,
    CMD = WEB_MIDI_LIB.midiEvent.COMMAND,
    ...
    Sequence.prototype = (function (window)
    {
        ...
        tick = function ()
        {
            ...
            while (delay <= PREQUEUE)
            {
                ...
                if (msg.isEmpty === undefined) // the first message in a rest symbol can have an isEmpty attribute 
                {
                    ...
                    // the sent timestamp needs to be absolute DOMHRT time when the message is sent.
                    domhrtTimestamp = msg.timestamp += domhrtMsOffsetAtStartOfSequence;
                    midiOutputDevice.send(msg.data, domhrtTimestamp);
                }
        ...
        // which is called when the last MIDIMessage in the sequence or subsequence has been sent.
        //
        ...
        getSubsequences = function (livePerformersTrackIndex)
        {
            ...
            function getEmptySubsequences(livePerformersTrack)  // 'base' function in outer scope.
            {
                ...
                for (i = 0; i < nPerformersMidiMoments; ++i)
                {
                    ...
                    {
                        s = new WEB_MIDI_LIB.sequence.Sequence(midiMoment.messages[0].msPositionInScore);
            ...
            function fillSubsequences(subsequences, midiMoments)  // 'base' function in outer scope.
            {
                ...
                for (subsequencesIndex = 0; subsequencesIndex < nSubsequences; ++subsequencesIndex)
                {
                    ...
                    track = new WEB_MIDI_LIB.track.Track();
                ...
                for (i = 1; i < nSubsequences; ++i)
                {
                    ...
                    if (lastNewS.restSubsequence !== undefined && subsequences[i].restSubsequence !== undefined)
                    {
                        ...
                        for (t = 0; t < nTracks; ++t)
                        {
                            ...
                            nMoments = trackToAppend.midiMoments.length;
                            for (iMom = 0; iMom < nMoments; ++iMom)
                            {
                                newMoment = trackToAppend.midiMoments[iMom];
                                ...
                                if (messages[0].isEmpty === undefined)
                                {
                                    ...
                                    {
                                        messages[iMsg].timestamp = newMoment.timestamp;
                                    }
        ...
        {
            var i, finalBarlineMoment, restMsg;

            finalBarlineMoment = new WEB_MIDI_LIB.midiMoment.MIDIMoment(timestamp);
            ...
            finalBarlineMoment.addMIDIMessage(restMsg);

            track.addMIDIMoment(finalBarlineMoment, 0);
        },

        // Returns a new restSequence equal to this one upto (but not including) toMs,
        ...
        beforeSplit = function (toMs)
        {
            ...
            limit = toMs - this.msPositionInScore;

            for (t = 0; t < nTracks; ++t)
            {
                newTrack = new WEB_MIDI_LIB.track.Track();
                track = tracks[t];
                nMoments = track.midiMoments.length;
                for (iMom = 0; iMom < nMoments; ++iMom)
                {
                    momentToAppend = track.midiMoments[iMom];
                    if (momentToAppend.timestamp >= limit)
                    {
                        break;
                    }

                    newTrack.addMIDIMoment(momentToAppend, 0);
                }

                appendFinalBarlineMoment(newTrack, this.msPositionInScore, limit);
        ...
        afterSplit = function (fromMs)
        {
            ...
            for (t = 0; t < nTracks; ++t)
            {
                ...
                newMoment = new WEB_MIDI_LIB.midiMoment.MIDIMoment(0);
                ...
                if (nMoments > 0)
                {
                    ...
                    for (iMom = momentI; iMom < nMoments; ++iMom)
                    {
                        ...
                        newMoment = new WEB_MIDI_LIB.midiMoment.MIDIMoment(moment.timestamp + this.msPositionInScore - fromMs);
        ...
        finishSilently = function ()
        {
            ...
            while (msg !== null)
            {
                if (!((msg.command === NOTEON_COMMAND && msg.data2 > 0) || msg.isEmpty !== undefined))
                {
                    ...
                    midiOutputDevice.send(silentMsg);
                    ++i;
                }
                msg = nextMessage();
            }
