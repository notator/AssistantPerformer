...
MIDI_API.sequence = (function (window)
{
    ...
    Sequence.prototype = (function (window)
    {
        ...
        nextMoment = function ()
        {
            ...
            if (!stopped && !paused)
            {
                ...
                if (nextMomt !== null)
                {
                    ...
                    if (nextMomt.msPositionInScore === endMarkerMsPositionInScore && nextMomt.msPositionInScore < finalBarlineMsPositionInScore)
                    {
                        nextMomt = null; // call stop() (below).
                    }
                    else
                    {
        ...
        resume = function ()
        {
            ...
            {
                var
                i, tracks = that.tracks, nTracks = tracks.length, track,
                j, nMoments, moment;
        ...
        playSpan = function (midiOutDevice, fromMsPositionInScore, toMsPositionInScore, trackIsOnArray,
                                speed, recording, reportEndOfSeq, reportMsPosition)
        {
            ...
            // Sets the timestamps of the moments in the tracks to absolute DOMHRT values.
            function setTimestamps(tracks, startNow, speed)
            {
                var i, j, k, moment, track, nTracks = tracks.length, nEvents, firstMsPosition;

                function getFirstMsPosition(tracks)
                {
                    var
                    i, nTracks = tracks.length,
                    firstTrackMsPosition, firstMsPosition = Number.MAX_VALUE;
                    ...
                    for (i = 0; i < nTracks; ++i)
                    {
                        ...
                        {
                            firstTrackMsPosition = track.moments[track.fromIndex].msPositionInScore;
                            firstMsPosition = (firstMsPosition < firstTrackMsPosition) ? firstMsPosition : firstTrackMsPosition;
                        }
                    }
                    return firstMsPosition;
                }

                firstMsPosition = getFirstMsPosition(tracks);
                for (i = 0; i < nTracks; ++i)
                {
                    track = tracks[i];
                    for (j = track.fromIndex; j <= track.toIndex; ++j)
                    {
                        ...
                        moment.timestamp = startNow + ((moment.msPositionInScore - firstMsPosition) / speed);
                    }
                }
            }
            ...
            startNow = window.performance.now(); // startNow is also used to calculate the total duration of the performance
            setTimestamps(that.tracks, startNow, speed);
        ...
        publicPrototypeAPI =
        {
            ...
            revertTimestamps: revertTimestamps
        };
