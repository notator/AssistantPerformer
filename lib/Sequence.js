...
MIDI_API.sequence = (function (window)
{
    ...
    Sequence.prototype = (function (window)
    {
        ...
        stop = function ()
        {
            ...
            {
                setState("stopped");
                reportEndOfSequence(recordedSequence, false);
            }
        },
        ...
        beforeSplit = function (toMs)
        {
            ...
            returnSeq = new Sequence(this.msPositionInScore),
        ...
        afterSplit = function (fromMs)
        {
            ...
            for (t = 0; t < nTracks; ++t)
            {
                ...
                newEvt.msPositionInScore = this.msPositionInScore;
                newEvt.isEmpty = true;
                ...
                if (nMoments > 0)
                {
                    momentI = indexOfLastMomentBeforeFromMs(track.moments, fromMs - this.msPositionInScore);
                    if (momentI === undefined)
                    {
                        // track.moments[0].timestamp was greater than (fromMs - this.msPositionInScore)
                        // i.e. copy *all* the subsequent events to the new track.
                        momentI = 0;
                    }
                    else if (track.moments[momentI].timestamp + this.msPositionInScore < fromMs)
                    {
                        ++momentI;
                    }
                    for (iMom = momentI; iMom < nMoments; ++iMom)
                    {
                        ...
                        newMoment = new MIDI_API.moment.Moment(moment.timestamp + this.msPositionInScore - fromMs);
                        if (moment.restStart !== undefined)
                        {
                            newMoment.restStart = true;
                        }
