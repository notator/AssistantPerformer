...
MIDILib.sequence = (function (window)
{
    ...
    Sequence.prototype = (function (window)
    {
        ...
        pausedAtMsPositionInScore = null, // set by pause(), used by resume()
        ...
        setState = function (state)
        {
            switch (state)
            {
                case "stopped":
                    stopped = true;
                    paused = false;
                    pausedAtMsPositionInScore = null;
                    break;
                case "paused":
                    stopped = false;
                    paused = true;
                    pausedAtMsPositionInScore = currentMoment.msPositionInScore;
                    break;
                case "running":
                    stopped = false;
                    paused = false;
        ...
        nextMoment = function ()
        {
            ...
            if (!stopped && !paused)
            {
                ...
                else
                {
                    ...
                    if (nextMomt.timestamp === UNDEFINED_TIMESTAMP)
                    {
                        // The usual case.
                        // Each moment's msPositionInScore takes account of a global speed setting.
                        nextMomt.timestamp = nextMomt.msPositionInScore + startTimeOffset;
                    }
                    else
                    {
                        // If the speed is changing during the performance, each moment's timestamp
                        // is set (relative to the start of this sequence, using the moment's
                        // msPositionInScore and the current speed) before calling playSpan().
                        nextMomt.timestamp += startTimeOffset;
                    }
                }
            }
        ...
        tick = function ()
        {
            ...
            while (delay <= PREQUEUE)
            {
                ...
                if (currentMoment.messages.length > 0) // rest moments can be empty (but should be reported above) 
                {
                    ...
                    {
                        currentMoment.timestamp = MIDILib.constants.OTHER.UNDEFINED_TIMESTAMP;
                    }
                }
        ...
        // Should only be called when this sequence is paused and pausedAtMsPositionInScore is not null.
        resume = function ()
        {
            if (isPaused())
            {
                runFrom(pausedAtMsPositionInScore, window.performance.now());
            }
            else
            {
