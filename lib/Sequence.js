...
MIDI_API.sequence = (function (window)
{
    ...
    Sequence.prototype = (function (window)
    {
        ...
        sequenceMsDuration = -1,  // set in playSpan(), used by nextEvent()
        lastSequenceTimestamp = -1, // set in playSpan(), used by nextEvent()
        recordedSequence, // set in playSpan(), used by tick()
        ...
        tick = function ()
        {
            ...
            while (delay <= PREQUEUE)
            {
                ...
                {
                    recordedSequence.tracks[evt.channel()].addEvent(evt, recordedSequence.msPositionInScore);
        ...
        playSpan = function (midiOutDevice, fromMs, toMs, trackIsOnArray, reportEndOfSeq, reportMsPosition)
        {
            ...
            sequenceMsDuration = toMs - fromMs;
        ...
        getSubsequences = function (livePerformersTrackIndex)
        {
            ...
            {
                fillSubsequences(subsequences, that.tracks[trackIndex].moments);
            }
        ...
        beforeSplit = function (toMs)
        {
            ...
            that = this,
            nTracks = that.tracks.length,
            returnSeq = new Sequence(that.msPositionInScore),
        ...
        afterSplit = function (fromMs)
        {
            ...
            if (that.msPositionInScore >= fromMs || this.restSubsequence === undefined)
            {
            ...
            for (t = 0; t < nTracks; ++t)
            {
                ...
                Object.defineProperty(newEvt, "msPositionInScore", { value: that.msPositionInScore, writable: false });
                newEvt.isEmpty = true;
                ...
                if (nMoments > 0)
                {
                    ...
                    for (iMom = momentI; iMom < nMoments; ++iMom)
                    {
                        ...
                        newMoment = new MIDI_API.moment.Moment(moment.timestamp + that.msPositionInScore - fromMs);
                        if (moment.restStart !== undefined)
                        {
                            newMoment.restStart = true;
                        }
