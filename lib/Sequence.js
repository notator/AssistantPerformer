...
MIDILib.sequence = (function (window)
{
    ...
    Sequence.prototype = (function (window)
    {
        ...
        resume = function ()
        {
            if (isPaused() && pausedAtMsPositionInScore !== null)
            {
                runFrom(pausedAtMsPositionInScore, window.performance.now());
            }
            else
            {
                throw "Error: resume() should only be called when this sequence is paused and pausedAtMsPositionInScore is not null.";
            }
        },
