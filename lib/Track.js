...
MIDI_API.track = (function ()
{
    ...
    // If sequenceMsPositionInScore is defined, a clone of the event is contructed, and
    // the sequenceMsPositionInScore is subtracted from the clone's timestamp before
    // adding the clone to the track.
    // This is done for recordings, when recording begins after the beginning of a score.
    Track.prototype.addEvent = function (event, sequenceMsPositionInScore)
    {
        var eventToAdd = event, moment;

        if (sequenceMsPositionInScore !== undefined && sequenceMsPositionInScore >= 0)
        {
            eventToAdd = event.clone();
            eventToAdd.timestamp -= sequenceMsPositionInScore;
        }

        if ((this.moments.length === 0)
        ||  (this.moments[this.moments.length - 1].timestamp < eventToAdd.timestamp))
        {
            moment = new Moment(eventToAdd.timestamp);
            this.moments.push(moment);
        }
        else if (this.moments[this.moments.length - 1].timestamp === eventToAdd.timestamp)
        {
