 ...
 *
 *  jiTrack.js
 *  The JI_WEB_MIDI_API.track namespace which defines the
...
JI_WEB_MIDI_API.namespace('JI_WEB_MIDI_API.track');

JI_WEB_MIDI_API.track = (function ()
{
    "use strict";

    var
    Moment = JI_WEB_MIDI_API.moment.Moment,
    ...
    Track.prototype.addMoment = function (moment, subsequenceMsPositionInScore)
    {
        ...
        {
            var i, nMessages = moment.messages.length;

            moment.timestamp -= subsequenceMsPositionInScore;
            for (i = 0; i < nMessages; ++i)
            {
                moment.messages[i].timestamp -= subsequenceMsPositionInScore;
            }
        }
