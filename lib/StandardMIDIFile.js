...
MIDI_API.standardMIDIFile = (function ()
{
    ...
    //      Moment.events is an array of Event (all events in a moment have the same timestamp)
    //      Event.data is an array of byte, (the bytes in the data are in sequence in the file)
    //      Event.timestamp
    //          Usually, event.timestamps are the (floating point) number of milliseconds from
    //          the beginning of a score, and the default event.timestamp is their containing
    //          moment's msPositionInScore. Event.timestamps can however change as a result of
    //          changing the playback speed or being in an assisted performance.
    //          When this function is called, the timestamps have been shifted to be relative
    //          to the beginning of the (recorded) argument sequence. The smallest timestamp is
    //          therefore always 0 here.
    //          This function stores timings in milliseconds (packed into a small number of bytes,
    //          according to standard MIDI packing algorithm). In Standard MIDI files, timings
    //          _precede_ their event, and are relative to the time of the _previous_event_.
    //      The sequence's tracks currently do not include the events which are automatically
    //      written into the file at the end of each track. These events (which all happen 
    //      "synchronously") are AllControllersOff, AllSoundOff and EndOfTrack.
    // sequenceMsDuration is the total duration of the sequence in milliseconds (an integer),
    //      and determines the timing of the end-of-track events.
    sequenceToSMF = function (sequence, sequenceMsDuration)
    {
        ...
        // Event timestamps are relative to the start of the tracks.
        // The earliest event timestamp is 0.
        ...
        function sequenceToUint8Array(tracks, sequenceMsDuration)
        {
            ...
            function getTrackChunk(trackMoments, sequenceMsDuration)
            {
                ...
                // Start the track 200 milliseconds before the first event in any track,
                // and end it 300 milliseconds after the end of the sequence 
                // so that the start and end of playback are not quite so abrupt.
                trackData = getTrackData(-200, trackEvents, sequenceMsDuration + 300);
