 ...
 *
 *  lib/Message.js
 *  The MIDI_API.message namespace which defines
 ...
 *      3. function getMessage(data) for constructing any kind of message. This function
 *         can be used to construct messages when their type is not known in advance (e.g. when
...
MIDI_API.namespace('MIDI_API.message');

MIDI_API.message = (function ()
{
    "use strict";
    var
    ...
    // Use the factory function getMessage(data) if the message type is unknown in
    // advance (for example when reading an input stream).
    // getMessage() can create SysEx and "running data" messages too.
    ...
    // If data is undefined, returns an empty object. Otherwise a 1, 2 or 3-byte Message,
    // or a SysExMessage, depending on the data.
    // An exception will be thrown if the data is illegal.
    // data must be omitted (=undefined) or an array of numbers in range 0..0xF0.
    getMessage = function (data)
    {
        ...
        else if (data[0] === SYSTEM_EXCLUSIVE.START)
        {
            ...
            {
                message = new SysExMessage(data);
            }
            else
            {
                throw "Error in System Exclusive message.";
            }
        }
        else if ((data[0] & 0xF0) === 0xF0)
        {
            ...
            {
                message = new Message(data[0], 0, 0);
            }
            else
            {
        ...
        {
            message = new Message(data[0], data[1], 0);
        }
